...
# INSTALA PACOTES DE COMPILAÇÃO (Isso é especialmente útil para desenvolvedores que desejam instalar software que não está disponível nos repositórios)
sudo apt install build-essential -y

# fornecer versões mais recentes do Python que não estão disponíveis nos repositórios oficiais do Ubuntu ou que são mantidas para versões mais antigas do
# Ubuntu. Isso é particularmente útil para desenvolvedores que precisam de versões específicas do Python para seus projetos ou para testar seus programas em
# diferentes versões do Python
sudo add-apt-repository ppa:deadsnakes/ppa

# Instala o Python e o Ambiente Virtual
sudo apt install python3.11 python3.11-venv -y
OBS: para testar se instalou podemos digitar -> python3.11 -V

# Instala o servidor NGINX
sudo apt install nginx -y

# Para gerar certrificados SSL fornecidos pela pelo Let's Encrypt gratuitamente
